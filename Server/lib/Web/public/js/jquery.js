...
/*! jQuery v1.9.0 | (c) 2005, 2012 jQuery Foundation, Inc. | jquery.org/license */ (function (
  ...
  (st.fn = st.prototype =
    ...
    (st.ready.promise = function (t) {
      if (!U)
        if (((U = st.Deferred()), "complete" === V.readyState)) _st(st.ready);
  ...
  (st.Animation = st.extend(P, {
    ...
    (st.fx.timer = function (e) {
      e() && st.timers.push(e) && st.fx.start(e);
    }),
    (st.fx.interval = 13),
    (st.fx.start = function (t) {
      Kn || ((Kn = _si(st.fx.tick, st.fx.interval)), (t.id = Kn));
    }),
    (st.fx.stop = function () {
...
// [160731] fx.start 에서 timerId(=Kn)를 timer(t, 외부에서 e) 객체에 삽입하도록 수정 / 타이머 지중화
